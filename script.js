...
function showChallengeResult(targetIndex) {
  ...
  if (resultCard && resultTitle && resultAction) {
...
function spinChallengeWheel() {
  if (isWheelSpinning || currentTasks.length === 0 || !wheelElement || !spinWheelButton) {
    return;
  }
...
if (wheelElement && wheelLabels && spinWheelButton) {
  refreshChallengeSet();
